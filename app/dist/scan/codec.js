...
var UnreadableScanError = class extends Error {
	constructor(id, why) {
...
function decodeScan(id, text) {
	let file;
	try {
		file = JSON.parse(text);
	} catch (error) {
		throw new UnreadableScanError(id, `it is not valid JSON (${error.message})`);
	}
	if (!READABLE.has(file.codecVersion)) throw new UnreadableScanError(id, `it was written by encoding version ${String(file.codecVersion)} and this build reads ${[...READABLE].join(" and ")}. Scans written by a version this build does not know are not read rather than read approximately.`);
	const raw = file.scan;
	if (raw?.id == null || raw.stamp == null || raw.score == null) throw new UnreadableScanError(id, "it is missing the id, stamp or score a scan must have");
	...
	return {
		...
		signals: raw.signals.map((signal) => ({
			...signal,
			collectedAt: date(signal.collectedAt, id, "signal.collectedAt")
		}))
	};
}
/**
...
function score(raw) {
	...
	return {
		...rest,
		pillars,
		composition: composition ?? mixture(raw.scoredControls, attestedControls ?? 0)
	};
}
/** A scored set that was observed except where it was answered, which is what those runs were. */
function mixture(scored, attested) {
	return {
		observed: Math.max(0, scored - attested),
		"admin-collected": 0,
		attested
	};
}
...
*/
function measurement(raw, findings, finishedAt, id) {
	if (raw.measurement != null) return raw.measurement.map((entry) => ({
		...entry,
		measuredAt: date(entry.measuredAt, id, "measuredAt")
	}));
...
*/
function footprint(raw, id) {
	const stored = raw.footprint;
	if (!isRecord(stored)) throw new UnreadableScanError(id, "it carries no footprint, and the run record is read from one");
	const spend = stored.spend;
	if (!isRecord(spend) || !isRecord(spend.spent) || !isRecord(spend.limits) || typeof spend.elapsedMs !== "number") throw new UnreadableScanError(id, "its footprint's spend is not the shape a run record reads: spent, limits and a numeric elapsedMs");
	const tasks = stored.tasks;
	if (!isRecord(tasks)) throw new UnreadableScanError(id, "its footprint counts no tasks, and every surface on the record is read from them");
	for (const [surface, counters] of Object.entries(tasks)) {
		if (!isRecord(counters)) throw new UnreadableScanError(id, `its footprint's ${surface} counters are not a record of counts`);
		const absent = COUNTS.filter((count) => typeof counters[count] !== "number");
		if (absent.length > 0) throw new UnreadableScanError(id, `its footprint's ${surface} counters have no ${absent.join(", ")}`);
		if (!isRecord(counters.terminal)) throw new UnreadableScanError(id, `its footprint's ${surface} counters have no terminal, so it was written by a build before row 81 and stored under an encoding version that does not distinguish the two`);
	}
	if (!isRecord(stored.limiters)) throw new UnreadableScanError(id, "its footprint's limiters are not a record, and the record counts reductions from them");
...
function revive(finding, id) {
	return {
		...
		evidence: finding.evidence.map((evidence) => ({
			...evidence,
			collectedAt: date(evidence.collectedAt, id, "evidence.collectedAt")
		}))
	};
}
/**
...
*/
function date(value, id, field) {
	const parsed = value instanceof Date ? value : new Date(value);
	if (Number.isNaN(parsed.getTime())) throw new UnreadableScanError(id, `${field} is not a date (${JSON.stringify(value)})`);
	return parsed;
}
//#endregion
export { UnreadableScanError, decodeScan, encodeScan };
