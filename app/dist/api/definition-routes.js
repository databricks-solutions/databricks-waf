import { DefinitionError, archive, currentVersion, define, resolveScope, revise, unarchive } from "../define/definition.js";
import { DefinitionConflict } from "../define/store.js";
import { SetupError, ready, resumeAt, standingOf, troubles } from "../define/setup.js";
import { preflight } from "../define/preflight.js";
//#region server/api/definition-routes.ts
const NO_STORE = "This installation is not keeping assessment definitions, so there is nowhere to put one. Bind a database and restart, and the definitions you create will survive a deploy.";
const NO_DRAFTS = "This installation is not keeping unfinished assessments, so leaving this page loses what you have written. Bind a database and restart, and you will be able to come back to it.";
/**
...
*/
async function refuse(response, act, status, error, message, extra = {}) {
...
function registerDefinitionRoutes(app, options) {
	...
	*/
	...
	app.post("/api/definitions", async (request, response) => {
		...
		try {
			...
			await forget(options, actor, void 0);
	...
	*/
	app.post("/api/definitions/:id/versions", async (request, response) => {
		...
		try {
			...
			const permission = await options.permitted(request, response, "definition.revise", { target: {
				kind: "definition",
				id
			} });
			const { actor } = permission;
			...
			if (typeof from !== "number" || !Number.isInteger(from)) throw new DefinitionError("A revision has to say which version it was made from, so a change made against a copy somebody else has already replaced can be refused rather than applied.");
			const current = currentVersion(existing);
			if (from !== current.version) {
				await refuse(response, permission.act, 409, "stale-definition", `This was revised from version ${String(from)}, and version ${String(current.version)} is now current — ${current.createdBy} changed it. Re-read it and decide against what they changed.`, { currentVersion: current.version });
				return;
			}
	...
	app.post("/api/definitions/:id/preflight", async (request, response) => {
		...
		if (options.probeFor == null || options.sources == null) {
			response.status(503).json({
				...
				message: "This installation has no SQL warehouse bound, so there is nothing to check the grants against. Bind one and the preflight will run."
			});
			return;
		}
		let act;
		try {
			...
			const directory = latest == null ? void 0 : directoryFrom(latest.estate);
			const unreadable = directory == null ? latest?.estate.undeterminedReason : void 0;
			...
			}, options.probeFor(request, actor), now());
			await act.performed(target);
			response.json({
	...
	*/
	app.put("/api/definitions/drafts", async (request, response) => {
		...
		try {
			...
			await act.performed(draft.definitionId != null ? {
				kind: "definition",
		...
		try {
			...
			if (directory == null) {
				response.json({
					...
					outOfScope: 0,
					complete: false,
					description: "What this scope covers is not known yet, because nothing has read the account directory to hold it against.",
					...latest != null ? { asOf: latest.startedAt.toISOString() } : {},
					unavailable: latest == null ? "No scan has run yet, so there is no account directory to resolve this scope against. Run one, and this will say what the scope covers." : latest.estate.undeterminedReason ?? "The last scan could not read the account directory, so there is nothing to resolve this scope against."
				});
				return;
			}
			const resolution = resolveScope({
				scope,
				lookbackDays: MIN_LOOKBACK_FOR_PREVIEW
			}, directory);
			const named = {
			...
			if (latest == null) {
				...
				if (current?.directory == null) {
					response.json({
						...
						unavailable: current?.unavailable ?? "No scan has run yet, and this installation cannot read the account directory on demand."
					});
					return;
				}
				response.json({
...
*/
function directoryFrom(estate) {
	if (estate.undeterminedReason != null) return void 0;
	...
	const excluded = estate.excluded.map((workspace) => ({
		...row(workspace, false),
		reason: workspace.reason ?? "not-running"
	}));
	return {
...
function presentPreflight(result) {
	return {
		...
		...result.scope.undeterminedReason == null ? { scope: {
...
function selectable(workspace, assessable) {
	return {
		id: workspace.id,
		name: workspace.name,
		...workspace.url != null ? { url: workspace.url } : {},
...
function selectableDirectory(directory) {
	...
	return directory.workspaces.map((workspace) => selectable({
		id: workspace.workspaceId,
		name: workspace.name,
		...workspace.url != null ? { url: workspace.url } : {},
		status: workspace.status,
		...excluded.get(workspace.workspaceId) != null ? { reason: excluded.get(workspace.workspaceId) } : {}
	}, live.has(workspace.workspaceId))).sort((left, right) => left.name.localeCompare(right.name));
}
...
*/
async function forget(options, author, definitionId) {
	try {
...
*/
async function presentDraft(draft, options) {
	const definition = draft.definitionId == null ? void 0 : await read(options, draft.definitionId);
	const { standing, warning } = standingOf(draft, definition);
	const name = definition == null ? void 0 : currentVersion(definition).attribution.name;
	return {
		...
		ready: ready(draft),
		troubles: troubles(draft).map((one) => ({
			step: one.step,
			trouble: one.trouble
		})),
...
*/
function measurable(measurement, options) {
	...
	throw new DefinitionError(`This build does not measure ${unknown.join(", ")}. It measures ${measured.join(", ")}. An assessment that named a pillar nothing measures would report a score of the pillars it could measure, under a name that claimed more.`);
}
/**
...
*/
function targetable(targets, options) {
	const measured = options.pillars;
	...
	if (unknown.length === 0) return;
	throw new DefinitionError(`This build does not measure ${unknown.join(", ")}, so a target for ${unknown.length === 1 ? "it" : "them"} could never be reported against. It measures ${measured.join(", ")}.`);
}
async function read(options, id) {
	try {
...
*/
function draftContentFrom(body, author, savedAt) {
	const raw = asObject(body);
	const { definitionId, fromVersion, name, purpose, owners, lookbackDays, pillars, note } = raw;
	if (definitionId != null && typeof definitionId !== "string") throw new SetupError("A revision names the assessment it revises by id.");
	if (typeof definitionId === "string" && definitionId.trim() === "") throw new SetupError("A revision names the assessment it revises by id. Leave the id out altogether for a new assessment rather than sending an empty one.");
	if (definitionId != null && fromVersion == null) throw new SetupError("A revision has to say which version it was started from, so it can be told later that somebody else has changed the assessment since.");
	if (fromVersion != null && (typeof fromVersion !== "number" || !Number.isInteger(fromVersion))) throw new SetupError("A version is a whole number.");
	...
	if (lookbackDays != null && typeof lookbackDays !== "number") throw new SetupError("A lookback is a number of days.");
...
function draftTargetsFrom(raw) {
	...
	return raw.map((entry) => {
		...
		if (target.atLeast != null && typeof target.atLeast !== "number") throw new SetupError("A target is a score out of a hundred.");
...
* started to narrow.
*/
function draftScopeFrom(raw) {
	if (raw == null) return void 0;
	const scope = asObject(raw);
	if (scope.kind !== "account" && scope.kind !== "selected") throw new SetupError("A scope is either the whole account or a set of workspaces that was chosen.");
	const workspaceIds = scope.workspaceIds;
	if (workspaceIds != null && !isStrings(workspaceIds)) throw new SetupError("A selected scope names its workspaces by id.");
	return {
		kind: scope.kind,
...
function presentVersion(version) {
	...
	return {
		...
		...version.targets != null ? { targets: version.targets.map((target) => ({
			pillar: target.pillar,
...
function asObject(body) {
	if (body == null || typeof body !== "object" || Array.isArray(body)) throw new DefinitionError("An assessment is described by an object.");
	return body;
}
/**
...
function revisionFrom(body) {
	...
	if (body.measurement == null && body.attribution == null && body.targets == null) throw new DefinitionError("A revision has to change the measurement, the attribution or the targets.");
	return {
...
function targetsFrom(raw) {
	...
	return raw.map((entry) => {
		...
		if (typeof pillar !== "string" || pillar.trim() === "") throw new DefinitionError("A target names the pillar it is a target for.");
		if (typeof target.atLeast !== "number") throw new DefinitionError(`The target for ${pillar} has to say what score it commits to.`);
		if (typeof target.by !== "string" && !(target.by instanceof Date)) throw new DefinitionError(`The target for ${pillar} has to say by when.`);
		const by = target.by instanceof Date ? target.by : new Date(target.by);
		if (Number.isNaN(by.getTime())) throw new DefinitionError(`The date on the ${pillar} target could not be read as a date.`);
		return {
			pillar: pillar.trim(),
			atLeast: target.atLeast,
			by
...
function measurementFrom(raw) {
	...
	if (typeof lookbackDays !== "number") throw new DefinitionError("An assessment has to say how far back it looks.");
...
function scopeFrom(raw) {
	...
	if (scope.kind === "selected") {
		...
		if (named.length === 0) throw new DefinitionError("A selected scope names at least one workspace. An assessment that covers nothing measures nothing; to cover everything, scope it to the account instead.");
		return {
			kind: "selected",
			workspaceIds: named
		};
...
function attributionFrom(raw) {
	const { name, purpose, owners } = asObject(raw);
...
//#endregion
export { registerDefinitionRoutes };
