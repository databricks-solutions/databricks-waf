...
//#region server/plan/descriptors.ts
const RUNS_AS_USER = "whoever started the run, using the token the platform forwards to the app — a person who pressed the button, or the service principal a schedule runs as. The app never reads the estate as its own service principal, so an assessment shows only what the identity behind it is entitled to see";
const SURFACES = [
	{
		surface: "sql",
		title: "System tables",
...
function metastoreGrants(tables) {
	return schemasOf(tables).map((schema) => ({
		kind: "metastore-grant",
		what: `SELECT on ${schema}`,
		note: schema === "system.information_schema" ? "Readable by anyone who can use the metastore; no grant is normally needed." : "Granted on the system schema by a metastore admin. Without it the check reports itself unmeasured rather than failing the estate."
	}));
}
/**
...
*/
const SQL_OBSERVES = {
	"sql:estate.workspaces": "Which workspaces the account has and which are still running, so every other account-wide check can exclude the ones that are gone.",
	"sql:estate.compute_profile": "What each workspace spent over the lookback window, and on which kinds of compute.",
	"sql:compute.clusters": "Every cluster the account can see, with its policy, access mode, init scripts and runtime version.",
	"sql:compute.warehouses": "Every SQL warehouse, with its size, type, scaling range and auto-stop setting.",
	"sql:compute.node_utilization": "Average per-cluster CPU utilisation over the window, and how many clusters sat idle throughout it.",
	"sql:cost.attribution": "How much of the spend carries the tags that would let it be attributed to a team or workload.",
...
function sqlDescriptors(queries) {
	return Object.entries(SQL_SIGNAL_SOURCES).map(([id, source]) => {
		...
		return {
			...
			...id === "sql:estate.workspaces" ? {} : { derivedFrom: [DIRECTORY_SIGNAL] }
		};
	});
}
const PER_OBJECT = [{
	...
	reach: "metastore",
	observes: "Size, file count, partitioning, clustering and deletion vectors for each table in the sample, which is what the layout and fragmentation checks are decided on.",
	touches: ["DESCRIBE DETAIL on each sampled table"],
	cost: {
		kind: "per-object",
		objects: "table in the sample",
		ceiling: 50
	},
	...
	reach: "metastore",
	observes: "Whether predictive optimization is enabled on each catalog holding tables, and whether the setting is the catalog’s own or inherited.",
	touches: ["DESCRIBE CATALOG EXTENDED on each catalog holding tables"],
	cost: {
		kind: "per-object",
		objects: "catalog holding at least one table"
	},
...
function restDescriptors() {
	return PROBES.map((probe) => ({
		...
		requires: [{
			kind: "app-scope",
			what: probe.scope,
			grantable: probe.grantable
		}, {
			kind: "workspace-permission",
			what: probe.permission
		}]
	}));
}
/**
...
const CLOUD = [{
	...
	reach: "account",
	observes: "The cloud-side storage bill for the estate’s external locations, read with a Unity Catalog service credential. Off unless the install named one; absence is not a fail. Active bytes from the Delta log are a different and smaller number.",
	touches: ["Unity Catalog temporary service credentials", "S3 Storage Lens or CloudWatch, mapped to external locations"],
	cost: { kind: "one-call" },
	requires: [{
		kind: "metastore-grant",
...
function signalDescriptors(queries = new FileQuerySource()) {
	return [
		...sqlDescriptors(queries),
		...PER_OBJECT,
		...restDescriptors(),
		...CLOUD
	];
...
//#endregion
export { SURFACES, signalDescriptors };
