...
//#region server/resolve/resolvers/helpers.ts
/**
...
*/
function fromSignal(signal, controls, evaluate) {
	return {
		controls: [...controls],
		requires: [signal],
...
function fromSignals(required, controls, evaluate) {
	return {
		controls: [...controls],
		requires: [...required],
		resolve(spec, signals) {
...
function valueOf(context, signal) {
	return context.signals.get(signal).value;
}
/**
...
*/
function sourcedFrom(signals, resolver) {
	return {
		...resolver,
		requires: [...resolver.requires, ...signals]
	};
}
/**
...
*/
function enrichedBy(signals, resolver) {
	return {
		...resolver,
		enrichedBy: [...signals],
		resolve: (spec, collected) => resolver.resolve(spec, collected)
	};
}
/**
...
function evidenceFrom(context, signal, observed, expected) {
	const result = context.signals.get(signal);
	return {
		signal,
		observed,
...
*/
function classOf(result) {
	return result.provenance?.authority === "admin-cli" ? "admin-collected" : void 0;
}
/**
...
*/
function detailFrom(context, signal, observed) {
	return {
		...evidenceFrom(context, signal, observed),
		bearing: "detail"
	};
}
/**
...
*/
function qualifierIn(context) {
	const value = directoryIn(context);
	if (value == null || value.live.length < 2) return () => void 0;
	const names = new Map(value.workspaces.map((workspace) => [workspace.workspaceId, workspace.name]));
	return (resource) => {
		if (resource.workspaceId == null || resource.workspaceId === "") return void 0;
...
*/
function offenders(context, signal, lead, rows, locate, options = {}) {
	...
	return [{
		...detailFrom(context, signal, `${lead}: ${items.map(describeItem).join(", ")}` + (rest > 0 ? ` and ${rest.toLocaleString("en-US")} more` : "")),
		at: {
			lead,
			items,
			...rest > 0 ? { more: rest } : {}
...
*/
function bandOutcome(share, bands) {
...
function threshold(spec, name, fallback) {
	const value = spec.thresholds?.[name];
	return typeof value === "number" ? value : fallback;
}
function bandsOf(spec, fallback) {
	return {
...
function percent(share) {
	return share == null ? "not applicable" : `${Math.round(share * 1e3) / 10}%`;
}
...
*/
const MATERIAL_UNPRICED_SHARE = .01;
/**
...
*/
function priceBarrier(coverage, figure) {
	...
	if ((coverage.currencies ?? 1) > 1) return `The window's usage is priced in ${String(coverage.currencies)} currencies, so ${figure} would add unlike amounts.`;
	...
	if (share == null) return `${noun} ${verb} no matching list price, so ${figure} would be computed over an incomplete bill.`;
	if (1 - share < MATERIAL_UNPRICED_SHARE) return null;
	return `${noun} ${verb} no matching list price, and ${priceCoverageClause(coverage)}, so ${figure} would be computed over an incomplete bill.`;
}
/**
...
*/
function priceCoverageClause(coverage) {
	const share = coverage.leastPricedShare;
	if (share == null) return coverage.unpricedRecords <= 0 ? "every usage record priced" : `${agreeing(coverage.unpricedRecords, "usage record").noun} unpriced`;
	const unit = coverage.leastPricedUnit ?? "usage";
	const units = coverage.usageUnitCount ?? 1;
	return units > 1 ? `${percent(share)} of ${unit} quantity priced, the least covered of ${String(units)} usage units` : `${percent(share)} of ${unit} quantity priced`;
}
...
function satisfiedByArchitecture(reason, evidence = []) {
	return {
...
*/
function unmeasured(reason, kind) {
	return {
		outcome: "unmeasurable",
		evidence: [],
		outcomeReason: reason,
		...kind != null ? { unmeasured: kind } : {}
	};
}
/**
...
*/
function agreeing(count, noun, plural = `${noun}s`) {
	return {
...
*/
const TRIGGER_COLUMNS_WRITTEN_FROM = /* @__PURE__ */ new Date("2026-01-01T00:00:00.000Z");
/**
...
*/
function multipleTriggers(job) {
	return job.triggerType === "MULTIPLE";
}
/** Whether a job's individual trigger mechanisms can be named from this row. */
function triggerMechanismRecorded(job) {
	return triggerRecorded(job) && !multipleTriggers(job);
}
//#endregion
export { agreeing, bandOutcome, bandsOf, bytes, detailFrom, directoryIn, enrichedBy, evidenceFrom, fromSignal, fromSignals, money, multipleTriggers, nameIn, notApplicable, observedValue, offenders, percent, priceBarrier, priceCoverageClause, qualifierIn, satisfiedByArchitecture, sourcedFrom, threshold, triggerMechanismRecorded, triggerRecorded, unmeasured, valueOf };
