...
//#region server/resolve/remedy.ts
/**
...
*/
function remedyFor(required, signals, context = {}) {
	const failed = required.filter((id) => (signals.get(id)?.status ?? "unmeasurable") === "unmeasurable");
	if (failed.length === 0) return void 0;
	return {
		...failed.map((id) => one(id, signals.get(id)?.unmeasurableReason, context)).reduce((so_far, candidate) => rank(candidate.kind) > rank(so_far.kind) ? candidate : so_far),
		signals: failed
	};
}
/**
...
function one(signal, reason, context) {
	const base = {
		signals: [signal],
	...
	if (reason == null) return {
		...base,
		kind: "report",
		says: `The ${signal} signal reported no reason for failing, so there is nothing here to act on. That is a defect in this app rather than a finding about your estate.`
	};
	const scope = demandedScope(reason);
	if (scope != null) return fromScope(signal, scope, context, base);
	if (/cancell?ed|timed out|timeout|budget|deadline|aborted/i.test(reason)) return {
		...base,
		kind: "retry",
		says: `The ${signal} signal did not finish in this run. Nothing about your estate is implied — run the scan again, and if it stops here repeatedly the surface budget is the thing to raise.`
	};
	if (/METASTORE_NOT_ASSIGNED|no metastore|not enabled|SCHEMA_NOT_FOUND|TABLE_OR_VIEW_NOT_FOUND|does not exist|UNRESOLVED_/i.test(reason)) return {
		...base,
		kind: "enable",
		says: `The source behind ${signal} is not present in this workspace. System schemas are enabled per metastore and per schema rather than all at once, so this usually means one has not been enabled yet rather than that anything is misconfigured.`
	};
	if (/PERMISSION_DENIED|permission|forbidden|not authorized|unauthorized|access denied|INSUFFICIENT_PERMISSIONS|\b403\b/i.test(reason)) return {
		...base,
		kind: "grant",
		says: `The identity this scan ran as was refused ${signal}. This one is inside your own estate: a metastore or table grant to the scanning identity closes it, and the requirement becomes measured on the next run.`
	};
	return {
		...base,
		kind: "report",
		says: `The ${signal} signal failed for a reason this app does not recognise, so it cannot say what would fix it. The platform's words are below; a copy of them is what an issue against this app needs.`
	};
}
/**
...
*/
function fromScope(signal, scope, context, base) {
	const family = familyOf(context.collector);
	if (family != null && !family.grantable) return {
		...base,
		kind: "attest",
		says: family.plane === "account" ? `${family.label} is account-plane configuration, and this app is installed in a workspace. The token is rejected before authorisation is considered, so no scope and no grant would help. Answer this requirement on the Answers page instead.` : `Reading this needs the "${scope}" scope, which Databricks Apps does not offer an app. No install of this app can be granted it, so answer this requirement on the Answers page rather than waiting for a version that measures it.`
	};
	if (context.declaredScopes?.includes(scope) === true) return {
		...base,
		kind: "re-authorise",
		says: `This app asks for the "${scope}" scope and the token it was handed does not carry it, which means your consent predates the request. Consent is per user and widening the requested set does not re-prompt anyone who already agreed, so signing in to the app again is what fixes this — for you, in one click, with no ticket.`
	};
	return {
		...base,
		kind: "report",
		says: `Reading ${signal} was refused for want of the "${scope}" scope, which this app does not declare. That is our omission rather than a limit of the platform or a gap in your estate: the scope belongs in the app manifest.`
	};
}
//#endregion
export { attestRemedy, remedyFor };
